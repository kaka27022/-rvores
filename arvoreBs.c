...
void inserir_(Pagina_ **raiz, Registro *item, Metrica* metrica)
{
    bool cresceu;
    int chave_retornada;
    ...
    if(*raiz != NULL)
    {
        ins_(*raiz, item, &pagina_retornada, &chave_retornada, &cresceu, metrica);
...
static bool pesquisaBinaria(Registro *pagina, int tamanho, int chave, Registro *reg_retorno, Metrica *metricas)
{
...
static bool pesquisa(Pagina_ *raiz, int chave, Registro *reg_retorno, Metrica *metricas)
{
    short int i;

    if(raiz->tp == FOLHA)
        return pesquisaBinaria(raiz->Uniao.Folha.registros, raiz->Uniao.Folha.n, chave, reg_retorno, metricas);
    ...
    if(chave < raiz->Uniao.Indice.indices[i - 1])
        return arvoreB_(raiz->Uniao.Indice.prox_pagina[i - 1], chave, reg_retorno, metricas);

    return arvoreB_(raiz->Uniao.Indice.prox_pagina[i], chave, reg_retorno, metricas);
}
...
bool arvoreB_(Pagina_ *raiz, int chave, Registro *reg_retorno, Metrica *metricas)
{
    ...
    inicio = clock();
    resultado_pesquisa = pesquisa(raiz, chave, reg_retorno, metricas);
    fim = clock();
